...
void KMeansBenckmark(const Matrix<float> DATABASE, int n_centroids, double *time_arr, double *sd_arr, int iter){
    ...
    for(int i = 0; i < 7; ++i){
        ...
        time_buff.push_back((time_end_v1 - time_start_v1) * 1e-9);
    }
    /**
     * sd = sqrt(1/(m-1)sum((x_i - x_bar)**2))
    */
    double mean = 0;
    for(const auto& elem : time_buff){
        mean += elem;
    }
    mean /= time_buff.size();
    double res = 0;
    for(const auto& elem : time_buff){
        res += std::pow(elem - mean, 2);
    }
    double sd = std::sqrt(1.0/(time_buff.size()-1)*res);

    //std::cout << mean << " (s) +- " << sd << " (s) 7 runs" << std::endl;
    printf("%d\r", iter);
...
int main(int argc, char** argv){
    ...
    for(int n = 0; n < size; ++n){
        KMeansBenckmark(DATABASE, n+2, time_arr, sd_arr, n);
    }
    std::cout << "\ntime_arr = [";
    for(int n = 0; n < size; ++n){
        std::cout << time_arr[n] << ", ";
    ...
    //unsigned __int64 time_start_v1 = std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
    //KM.run(100, 0);
    //std::cout << KM.getDataToCentroid() << std::endl;
    //unsigned __int64 time_end_v1 = std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
